...
def test_dinov2_onnx_model(model_path="output_models/dinov2_vitb_reg.onnx"):
    ...
    ort_session = ort.InferenceSession(model_path, providers=['WebGPUExecutionProvider'])
    print("ONNX Runtime session created successfully with WebGPU execution provider")
    ...
    dummy_input = np.random.randn(16, 3, 448, 448).astype(np.float32)
